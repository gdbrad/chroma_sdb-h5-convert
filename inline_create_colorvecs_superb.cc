...
{ 
  ...
  namespace InlineCreateColorVecsSuperbEnv 
  {
	...
	if (std::fabs(phase[i] - params.param.phase[i]) > 0)
      ...
      // add eigenvalue printing 
      QDPIO::cout << "Computing color vectors and eigenvalues" << std::endl;
        auto colorvecs_values = SB::ns_getColorvecs::computeColorvecs(
          u, params.param.t_start, params.param.Nt_forward, params.param.num_vecs, "cxyzXnt"
        );
        auto colorvecs = colorvecs_values.first;
        auto eigenvalues = colorvecs_values.second;

        // if (Layout::nodeNumber() == 0) {
          QDPIO::cout << "Printing computed eigenvalues:" << std::endl;
          for (int t = 0; t < params.param.Nt_forward; ++t) {
            for (int n = 0; n < params.param.num_vecs; ++n) {
              QDPIO::cout << "Eigenvalue for t= " << (params.param.t_start + t) % Lt << ", vec= " << n
                          << " : " << eigenvalues[t][n] << std::endl;
            }
          }
        // }
